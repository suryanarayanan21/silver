...
export const AppContext = createContext<{ _state: AppState; _dispatch: React.Dispatch<AppActionInternal>; } | null>(null);
...
export const usePiece = <PieceType extends Piece>(piece: PieceType) => {
    ...
    const dispatch = <ActionName extends keyof PieceType["actions"]>(action: { type: ActionName } & AppAction<PieceType, ActionName>) => {
...
export const StateProvider: FunctionComponent<{ children: JSX.Element }> = ({ children }) => {
    ...
    return <AppContext.Provider value={{ _state, _dispatch }}>
        {children}
    </AppContext.Provider>
}